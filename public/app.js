  ...
  panel.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="live-status p-live-status" style="display:none;">
        <div class="live-badge">🔴 البث شغال</div>
...
function initPreviewForPanel(clubKey, panel) {
  ...
  logoBox.addEventListener('pointermove', (e) => {
    ...
    newLeft = Math.max(0, Math.min(newLeft, frameRect.width - logoBox.offsetWidth));
    newTop = Math.max(0, Math.min(newTop, frameRect.height - logoBox.offsetHeight));
  ...
  resizeHandle.addEventListener('pointermove', (e) => {
    ...
    newW = Math.max(24, Math.min(newW, frameRect.width - logoBox.offsetLeft));
    newH = Math.max(24, Math.min(newH, frameRect.height - logoBox.offsetTop));
...
async function startLive(clubKey, container) {
  ...
  try {
    ...
    setLiveToast(container, '📡 البث بدأ', 'ok');
    refreshLiveStreams();
  } catch (err) {
