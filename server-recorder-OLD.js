...
function buildFfmpegArgs(streamUrl, outputPath, logo) {
...
/** بث HLS محلي مستمر (بدون حد زمني) — يقرأ المصدر مرة وحدة ويحط اللوقو مرة وحدة */
function buildLiveFfmpegArgs(streamUrl, hlsDir, logo) {
  const isHttp = /^https?:\/\//i.test(streamUrl);
  const args = ['-y'];
...
function startLiveProcess(club, streamUrl, hlsDir, logo) {
  const args = buildLiveFfmpegArgs(streamUrl, hlsDir, logo);
...
function startFfmpegProcess(id, streamUrl, outputPath, logo) {
  const args = buildFfmpegArgs(streamUrl, outputPath, logo);
  const proc = spawn('ffmpeg', args, { stdio: ['pipe', 'pipe', 'pipe'] });
  ...
  proc.on('close', (code) => {
    ...
    if (code !== 0) {
      ...
      // البث انتهى من طرف المصدر نفسه قبل أي إيقاف يدوي أو وقت مجدول — نرفع الملف تلقائيًا بدل ما يضيع
      notifyTelegram(`ℹ️ انتهى بث <b>${rec.matchName}</b> من طرف المصدر — جاري رفع الملف تلقائيًا`);
...
/** مراقبة حجم الملف وحد أقصى لمدة التسجيل — مشتركة بين التسجيل العادي وتسجيل شوط داخل بث مباشر */
function attachRecordingMonitor(id, rec) {
  rec.monitorHandle = setInterval(() => {
    fs.stat(rec.tempFilePath, (err, st) => {
      if (err) return;
      rec.fileSizeBytes = st.size;
      if (st.size !== rec.lastSize) {
...
function beginRecordingNow(id) {
  ...
  rec.tempFilePath = path.join(TEMP_DIR, `${id}.mp4`);
  const logo = rec.logoPath && rec.logoPos ? { path: rec.logoPath, ...rec.logoPos } : null;
  rec.process = startFfmpegProcess(id, rec.url, rec.tempFilePath, logo);
...
/** يوقف ffmpeg (لو شغال) ويرفع الملف بالخلفية بدون ما يحجب الرد على الطلب */
async function finalizeRecording(id) {
  ...
  if (rec.monitorHandle) clearInterval(rec.monitorHandle);

  rec.status = 'stopping';

  // ننتظر خروج ffmpeg فعليًا قبل الرفع (وإلا الملف يكون غير مكتمل)
  if (rec.process) {
    await new Promise((resolve) => {
      if (rec.process.exitCode !== null) return resolve();
  ...
  if (!rec.tempFilePath || !fs.existsSync(rec.tempFilePath) || fs.statSync(rec.tempFilePath).size === 0) {
    rec.status = 'error';
    rec.errorMessage = 'لم يُنتج أي ملف تسجيل — تأكد أن رابط البث صحيح ويعمل.';
    notifyTelegram(`❌ <b>${rec.matchName}</b> (${CLUBS[rec.club]})\n${rec.errorMessage}`);
    return;
  }
  ...
  try {
    const publicUrl = await uploadToB2(rec.tempFilePath, key);
    rec.status = 'done';
...
app.post('/api/preview/frame', requireApiKey, async (req, res) => {
  ...
  try {
    ...
    const frameArgs = ['-y', '-i', url.trim(), '-frames:v', '1', '-q:v', '3', framePath];
...
app.post('/api/live/start', requireApiKey, logoUpload.single('logo'), requireValidClub, (req, res) => {
  ...
  live.process = startLiveProcess(req.clubKey, live.url, hlsDir, logo);
  activeLiveStreams.set(req.clubKey, live);

  notifyTelegram(`📡 بدأ بث مباشر لـ <b>${CLUBS[req.clubKey]}</b>`);
  res.json({ success: true, hlsUrl: `/live/${req.clubKey}/stream.m3u8` });
});
...
app.post('/api/live/recording/start', requireApiKey, requireValidClub, (req, res) => {
  ...
  rec.process = startFfmpegProcess(id, localHlsUrl, rec.tempFilePath, null);
  attachRecordingMonitor(id, rec);
