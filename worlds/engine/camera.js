...
export class ThirdPersonCamera {
  constructor(THREE, { domElement, scene }) {
    ...
    // متجهات مؤقتة يُعاد استخدامها كل فريم بدل إنشاء Vector3 جديد كل مرة —
    // إنشاء كائنات جديدة 60 مرة بالثانية يثقّل على الـ Garbage Collector ويسبب بطء/تقطيع محسوس.
  ...
  update(targetPos, collisionMeshes) {
    ...
    const head = this._head.set(targetPos.x, targetPos.y + 1.5, targetPos.z);
    ...
    const camPos = this._camPos.copy(head).addScaledVector(dir, dist);
