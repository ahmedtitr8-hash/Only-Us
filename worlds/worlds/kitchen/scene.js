...
export function buildKitchen(THREE, interactionSystem) {
  ...
  const ROOM = 7.4; // نصف حجم الغرفة — مساحة أكبر بكثير من قبل عشان راحة الحركة واللعب
  const wallH = 2.9;

  // ---------- أرضية بلاط واقعية ----------
  const { map: tileMap } = tileFloorTextures(THREE, { repeat: 8 });
  ...
  // سجادة دافئة تحت الطاولة — تفصيلة واقعية بدون أي كلفة أداء (بلا ظل، مسطحة بالأرض)
  const { map: rugMap } = fabricTexture(THREE, 0x8a3f34, { repeat: 3 });
  const rug = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 2.6), new THREE.MeshStandardMaterial({ map: rugMap, roughness: 0.95 }));
  rug.rotation.x = -Math.PI / 2;
  rug.position.set(-2.4, 0.008, 0.3);
  ...
  const { map: wallMap } = plasterWallTexture(THREE, { repeat: 4 });
  ...
  // ---------- نافذتين واقعيتين: زجاج بتدرّج سماء + برواز + قضبان تقسيم ----------
  ...
  function addWindow(x, z, rotY) {
    ...
    const windowLight = new THREE.PointLight(0xdcefff, 0.32, 7, 2);
    const inward = new THREE.Vector3(0, 0, 1).applyAxisAngle(new THREE.Vector3(0, 1, 0), rotY);
    windowLight.position.set(x + inward.x * 1.2, 1.7, z + inward.z * 1.2);
    group.add(windowLight);
  }
  addWindow(-2.2, -ROOM + 0.1, 0);
  addWindow(2.6, -ROOM + 0.1, 0);

  // ---------- لمبة سقف معلّقة (تجميلية + مصدر إضاءة داخلية) ----------
  const lampGroup = new THREE.Group();
  lampGroup.position.set(-1.6, wallH, 1.6);
  ...
  lampGroup.add(bulb);
  const lampLight = new THREE.PointLight(0xffdca8, 0.9, 7.5, 2);
  lampLight.position.y = -0.68;
  // ملاحظة أداء مهمة: ظل PointLight يحتاج رندر المشهد 6 مرات (Cube Shadow Map) —
  // هذا أثقل شي ممكن يبطّئ الجوال. اللمبة تنوّر بدون ما "تُسقط" ظل كامل (كافي بصريًا هنا).
  lampLight.castShadow = false;
  lampGroup.add(lampLight);
  group.add(lampGroup);

  // لمبة سقف ثانية بالجهة اليمنى فوق الكاونتر — توزيع إضاءة أعدل بالغرفة الأكبر
  const lampGroup2 = lampGroup.clone();
  lampGroup2.position.set(ROOM - 1.6, wallH, -2.6);
  group.add(lampGroup2);
  ...
  function addCounter(x, z, w, d, h, rotY = 0) {
    ...
    top.castShadow = true;
    g.add(top);
    // خطوط فواصل أبواب الخزانة — بدونها الكاونتر يبان "صندوق واحد" كرتوني بلا تفاصيل
    const seamMat = new THREE.MeshStandardMaterial({ color: 0x000000, roughness: 0.9 });
    const doorCount = Math.max(1, Math.round(w / 0.42));
    for (let i = 1; i < doorCount; i++) {
      const seam = new THREE.Mesh(new THREE.BoxGeometry(0.006, h - 0.06, 0.006), seamMat);
      seam.position.set(-w / 2 + (w / doorCount) * i, h / 2, d / 2 + 0.006);
      g.add(seam);
    }
    group.add(g);
  ...
  // كل الكاونترات والأجهزة ملزّقة بالحائط اليمين فقط — يخلي وسط الغرفة فاضي تمامًا للحركة
  addCounter(ROOM - 0.45, -5.7, 0.85, 2.6, 0.9);
  addCounter(ROOM - 0.45, -2.6, 0.85, 2.6, 0.9);

  // ساعة حائط — تفصيلة صغيرة رخيصة بس تكسر فراغ الجدار وتحس إن المكان مسكون فعليًا
  const clockGroup = new THREE.Group();
  clockGroup.position.set(-4.4, 2.15, -ROOM + 0.08);
  const clockFace = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.03, 24), new THREE.MeshStandardMaterial({ color: 0xf6f2e8, roughness: 0.5 }));
  clockFace.rotation.x = Math.PI / 2;
  clockGroup.add(clockFace);
  const clockRim = new THREE.Mesh(new THREE.TorusGeometry(0.18, 0.012, 8, 24), new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.4, metalness: 0.4 }));
  clockGroup.add(clockRim);
  const handMat = new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: 0.5 });
  const hourHand = new THREE.Mesh(new THREE.BoxGeometry(0.011, 0.09, 0.01), handMat);
  hourHand.position.set(0, 0.03, -0.017); hourHand.rotation.z = 0.6; clockGroup.add(hourHand);
  const minHand = new THREE.Mesh(new THREE.BoxGeometry(0.009, 0.13, 0.01), handMat);
  minHand.position.set(0, 0.04, -0.018); minHand.rotation.z = -1.1; clockGroup.add(minHand);
  group.add(clockGroup);

  // مصفف أطباق صغير فوق الكاونتر — تفصيلة "حياة يومية" بسيطة وبدون كلفة أداء
  const rackGroup = new THREE.Group();
  rackGroup.position.set(ROOM - 0.45, 0.975, -4.6);
  const rackWireMat = new THREE.MeshStandardMaterial({ color: 0xb9b9b9, roughness: 0.35, metalness: 0.6 });
  for (let i = -2; i <= 2; i++) {
    const wire = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.006, 0.16, 6), rackWireMat);
    wire.position.set(i * 0.06, 0.08, 0);
    wire.rotation.x = 0.5;
    rackGroup.add(wire);
  }
  const plateColors = [0xffffff, 0xe7e2d6, 0xffffff];
  plateColors.forEach((clr, i) => {
    const plate = new THREE.Mesh(new THREE.CircleGeometry(0.09, 20), new THREE.MeshStandardMaterial({ color: clr, roughness: 0.35, side: THREE.DoubleSide }));
    plate.position.set(-0.12 + i * 0.12, 0.12, -0.02);
    plate.rotation.x = 0.5; plate.rotation.z = 0.06;
    rackGroup.add(plate);
  });
  group.add(rackGroup);
  ...
  const upperCabinet = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.75, 4.9), upperCabinetMat);
  upperCabinet.position.set(ROOM - 0.68, 2.05, -4.1);
  ...
  fridgeGroup.position.set(ROOM - 0.5, 0, 0.6);
  ...
  collisionBoxes.push(box(ROOM - 0.95, 0.22, ROOM - 0.05, 0.98));
  ...
  stoveGroup.position.set(ROOM - 0.45, 0.9, 2.6);
  ...
  // ---------- طاولة وكرسيّان بالجهة اليسرى (فوق السجادة) — يترك وسط ويمين الغرفة فاضي تمامًا ----------
  ...
  addChair(-2.4, 1.05, Math.PI);
  addChair(-2.4, -0.45, 0);

  // نبتة زينة بزاوية الغرفة — تفصيلة واقعية رخيصة (بدون ظل معقّد)
  ...
  plant.position.set(-ROOM + 0.6, 0, ROOM - 0.6);
  ...
  // مساحة الحركة الكبيرة الفاضية بمنتصف ويمين الغرفة هي أماكن الظهور — بعيدة عن كل الأثاث
  const spawnPoints = [
    { x: 1.8, z: 3.4, yaw: Math.PI },
    { x: 1.8, z: -2.2, yaw: Math.PI },
  ];
