...
import { fabricTexture, contactShadowTexture } from '../engine/textures.js';
...
export function buildCharacter(THREE, colorScheme) {
  ...
  // بشرة بلمعان خفيف جدًا (Clearcoat) يحاكي انعكاس الضوء الطبيعي عن الجلد بدل السطح المطفي البلاستيكي
  const skinMat = new THREE.MeshPhysicalMaterial({ color: colorScheme.skin, roughness: 0.55, metalness: 0, clearcoat: 0.18, clearcoatRoughness: 0.6 });
  ...
  head.add(headMesh);
  const hairMesh = new THREE.Mesh(new THREE.SphereGeometry(0.163, 18, 12, 0, Math.PI * 2, 0, Math.PI * 0.55), mat(hairColor, 0.85));
