...
// ================= استمرارية الجلسة (تنجينا من انقطاع التبويب أو تبديل التطبيقات) =================

function readSession() {
  try { return JSON.parse(sessionStorage.getItem('wt_session')); } catch (e) { return null; }
}
function writeSession(role, roomId, name) {
  sessionStorage.setItem('wt_session', JSON.stringify({ role, roomId, name }));
}

const storedSession = readSession();
let autoResume = null;

if (storedSession && storedSession.role === 'host' && (!roomFromUrl || roomFromUrl === storedSession.roomId)) {
  // جلسة مضيف بهذا التبويب تبقى مضيفة، طالما الرابط الحالي هو رابط غرفته نفسها أو بدون رابط
  autoResume = storedSession;
} else if (storedSession && storedSession.role === 'guest' && roomFromUrl && storedSession.roomId === roomFromUrl) {
  autoResume = storedSession;
}
...
enterBtn.addEventListener('click', () => {
  const chosenName = (nameInput.value || 'ضيف').trim();
  enterBtn.disabled = true;
  enterBtnText.textContent = 'جاري الاتصال';
  enterSpinner.classList.remove('hidden');

  if (roomFromUrl) {
    writeSession('guest', roomFromUrl, chosenName);
    beginAsGuest(roomFromUrl, chosenName);
  } else {
    const newId = randomId();
    writeSession('host', newId, chosenName);
    beginAsHost(newId, chosenName);
  }
});

// إعادة دخول تلقائي بدون شاشة البداية إذا كانت هذه الجلسة مستمرة من قبل
if (autoResume) {
  showRoom();
  if (autoResume.role === 'host') {
    beginAsHost(autoResume.roomId, autoResume.name);
  } else {
    beginAsGuest(autoResume.roomId, autoResume.name);
  }
}

function beginAsHost(roomId, name) {
  isHost = true;
  myName = name;
  hostRoomId = roomId;
...
function beginAsGuest(roomId, name) {
  isHost = false;
  myName = name;
  remotePeerId = roomId;
  nameMeLabel.textContent = myName;
...
function handlePeerError(err) {
  ...
  if (err.type === 'peer-unavailable') {
    ...
    // معرّف الغرفة مستخدم من جلسة قديمة، ننشئ معرّفًا جديدًا لنفس المضيف
    const newId = randomId();
    writeSession('host', newId, myName);
    beginAsHost(newId, myName);
  } else {
...
// إعادة اتصال تلقائية عند الرجوع للتبويب بعد تصفح تطبيق ثاني
