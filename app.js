...
const peerStatusRing = document.getElementById('peer-status-ring');
...
if (roomFromUrl) {
  ...
  modeHint.textContent = 'ستنضم إلى غرفة موجودة بمجرد الدخول';
} else {
  enterBtnText.textContent = 'إنشاء الغرفة';
  modeHint.textContent = 'ستحصل على رابط لمشاركته مع الطرف الآخر';
}
...
function setConnectionState(state, label) {
  peerStatusRing.className = 'status-ring ' + state;
  peerStatusLabel.textContent = label;
}
...
function createAsHost() {
  isHost = true;
  document.querySelector('#chip-me .participant-role').textContent = 'مضيف الغرفة';
  ...
  peer.on('open', (id) => {
    ...
    showRoom();
    setConnectionState('pending', 'بانتظار الانضمام');
  });
  ...
  peer.on('connection', (conn) => {
    ...
    conn.on('open', () => {
      ...
      setConnectionState('pending', 'جاري تأكيد الاتصال');
    });
  });
...
function joinAsGuest(roomId) {
  isHost = false;
  document.querySelector('#chip-me .participant-role').textContent = 'ضيف';
  remotePeerId = roomId;
  peer = new Peer({ config: RTC_CONFIG });

  peer.on('open', () => {
    showRoom();
    setConnectionState('pending', 'جاري الاتصال بالغرفة');
    ...
    const connectTimeout = setTimeout(() => {
      if (!(dataConn && dataConn.open)) {
        setConnectionState('error', 'تعذر الوصول للغرفة');
      }
    }, 12000);
...
function handlePeerError(err) {
  ...
  if (err.type === 'peer-unavailable') {
    setConnectionState('error', 'الغرفة غير متاحة');
  } else if (err.type === 'unavailable-id') {
    sessionStorage.removeItem('wt_host_room');
    createAsHost();
  } else {
    setConnectionState('error', 'خطأ بالاتصال');
  }
}
  ...
  dataConn.on('data', (msg) => {
    switch (msg.kind) {
      ...
      case 'seek':
        ...
        break;
      case 'peer-left':
        setConnectionState('pending', 'غادر الغرفة');
...
async function startMic() {
  try {
    ...
    micBtnLabel.textContent = 'تعذر الوصول للمايك';
    setTimeout(() => (micBtnLabel.textContent = 'المايك'), 2000);
    return;
  }
  micOn = true;
  ...
  micBtnLabel.textContent = 'المايك مفعّل';
...
function stopMic() {
  ...
  micBtnLabel.textContent = 'المايك';
  if (localStream) {
