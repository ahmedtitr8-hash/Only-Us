...
const DRIFT_THRESHOLD = 2.0; // ثانية: أعلى فرق نقبله قبل تصحيح المزامنة (لتقليل القفزات المزعجة)
  ...
  if (type === 'youtube') {
    ...
    if (type === 'hls' && window.Hls && Hls.isSupported()) {
      ...
      hlsInstance.on(Hls.Events.ERROR, (evt, data) => {
        if (data.fatal) showPlayerError('تعذر تشغيل هذا البث');
      });
      hlsInstance.loadSource(url);
      hlsInstance.attachMedia(nativePlayer);
    } else if (type === 'mpegts' && window.mpegts && mpegts.isSupported()) {
      mpegtsInstance = mpegts.createPlayer({ type: 'mpegts', isLive: false, url });
