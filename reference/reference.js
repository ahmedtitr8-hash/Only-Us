...
// ================= مصادر مشاهدة: عبر ووركر خاص (بدون بيانات حساب هنا) =================
// بيانات حساب اكستريم (username/password) ما عادت موجودة بهذا الملف — هي محفوظة
// كـ Secrets مشفّرة داخل الووركر نفسه على Cloudflare، وهذا الملف بس يكلّم الووركر.
// لو تبي تضيف أكثر من حساب/مصدر، سوّي ووركر ثاني لكل حساب وأضف رابطه هنا.
const XTREAM_SOURCES = [
  { name: 'qimyclient', workerBase: 'https://only-us.ahmedtitr8.workers.dev' },
];
...
async function xtreamApiFor(source, params = {}) {
  const url = new URL(source.workerBase.replace(/\/$/, '') + '/player_api.php');
...
function xtreamMovieUrl(item) {
  ...
  return `${src.workerBase.replace(/\/$/, '')}/movie/${item.stream_id}.${ext}`;
}

function xtreamEpisodeUrl(episode, sourceIndex) {
  const src = XTREAM_SOURCES[sourceIndex];
  const ext = episode.container_extension || 'mp4';
  return `${src.workerBase.replace(/\/$/, '')}/series/${episode.id}.${ext}`;
}
...
function toggleAccountItem(field) {
  ...
  const { id, mediaType, title, originalTitle, originalLanguage, posterPath, year, voteAverage } = currentDetailItem;
...
function goWatch(url) {
  window.location.href = `../index.html?video=${encodeURIComponent(url)}`;
}
