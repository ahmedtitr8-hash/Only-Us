...
async function fetchEmbedArabicResult(mediaType, tmdbId, season, episode) {
  ...
  for (const provider of EMBED_PROVIDERS) {
    try {
      const res = await fetch(`${EMBED_API_BASE}/api/streams/${provider}/${mediaType}/${tmdbId}${qs}`);
      if (!res.ok) continue;
      ...
      for (const s of streams) {
        ...
        if (arSub) {
          return {
            provider,
            videoUrl: embedProxyUrl('m3u8', s.url, s.headers),
            subUrl: embedProxyUrl('sub', arSub.url, {}),
            quality: s.quality || '',
            name: s.name || provider,
          };
        }
      }
    } catch (e) { /* نكمل للمزود اللي بعده */ }
  }
  return null;
}
...
function renderEmbedMovieResult(result) {
  ...
  embedResults.innerHTML = `
    <div class="ref-xtream-movie-row">
      <span class="ref-xtream-movie-name">${escapeHtml(result.name)}${result.quality ? ' — ' + escapeHtml(result.quality) : ''} (ترجمة عربية)</span>
...
async function searchEmbedMovie() {
  ...
  if (cacheKey in embedResultCache) {
    ...
    if (!cached) setEmbedStatus('ما لقينا ترجمة عربية لهذا الفيلم بهذا المصدر', 'err');
    else { setEmbedStatus(`تم: ${cached.name} (${cached.provider})`, 'ok'); renderEmbedMovieResult(cached); }
    return;
  }
  const token = ++embedToken;
  setEmbedStatus('جارِ البحث عن ترجمة عربية...', 'loading');
  try {
    const result = await fetchEmbedArabicResult('movie', id);
    embedResultCache[cacheKey] = result || null;
    if (token !== embedToken) return;
    if (!result) setEmbedStatus('ما لقينا ترجمة عربية لهذا الفيلم بهذا المصدر', 'err');
    else { setEmbedStatus(`تم: ${result.name} (${result.provider})`, 'ok'); renderEmbedMovieResult(result); }
  } catch (err) {
    if (token !== embedToken) return;
...
async function playEmbedEpisode(seasonNum, episodeNum, row, btn) {
  ...
  if (cacheKey in embedResultCache) {
    const cached = embedResultCache[cacheKey];
    if (!cached) { btn.title = 'ما لقينا ترجمة عربية لهذي الحلقة'; row.classList.add('embed-ep-failed'); }
    else goWatch(cached.videoUrl, cached.subUrl);
    return;
  }
