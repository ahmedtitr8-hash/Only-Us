...
// ================= مصدر إضافي: TMDB-Embed-API (سيرفرنا على Suga) =================
// يبحث عن روابط مشاهدة عند عدة مواقع بث ويرجّع لنا كمان ترجمات (منها العربي أحيانًا).
// نستخدم بروكسي السيرفر نفسه (m3u8-proxy / sub-proxy) عشان الفيديو والترجمة يشتغلوا
// من المتصفح بدون مشاكل CORS أو حظر Referer.
const EMBED_API_BASE = 'https://0zh990j42o17-production-pps4hflf.us-central1.suga.run';
const EMBED_PROVIDERS = ['castletv', 'netmirror', 'onetouchtv', 'streamflix', 'vixsrc'];

function embedProxyUrl(kind, url, headers) {
  const enc = encodeURIComponent(url);
  const h = headers && Object.keys(headers).length
    ? `&headers=${encodeURIComponent(JSON.stringify(headers))}`
    : '';
  return `${EMBED_API_BASE}/${kind}-proxy?url=${enc}${h}`;
}

// يدور بمزودي embed API واحد واحد ويرجع أول نتيجة عندها ترجمة عربية
async function fetchEmbedArabicResult(mediaType, tmdbId, season, episode) {
  let qs = '';
  if (mediaType === 'tv') qs = `?season=${encodeURIComponent(season)}&episode=${encodeURIComponent(episode)}`;
  for (const provider of EMBED_PROVIDERS) {
    const reqUrl = `${EMBED_API_BASE}/api/streams/${provider}/${mediaType}/${tmdbId}${qs}`;
    try {
      const res = await fetch(reqUrl);
      if (!res.ok) {
        console.warn(`[embed:${provider}] HTTP ${res.status} — ${reqUrl}`);
        continue;
      }
      const data = await res.json();
      const streams = (data && data.streams) || [];
      if (!streams.length) { console.warn(`[embed:${provider}] لا يوجد streams بالرد`, data); continue; }
      // نفضّل أي ستريم عنده ترجمة عربية، ولو ما فيه نرجّع أول ستريم صالح بدون ترجمة
      // (أفضل من ما نرجّع ولا شي — بعض المزودين مثل vixsrc ما يرجعون subtitles أصلاً بالرد)
      let best = null;
      let bestSub = null;
      for (const s of streams) {
        const subs = s.subtitles || [];
        const arSub = subs.find((sub) => /^ar/i.test(sub.lang || '') || /arab/i.test(sub.label || ''));
        if (arSub) { best = s; bestSub = arSub; break; }
        if (!best) best = s; // احتياط: أول ستريم صالح لو ما لقينا ترجمة بأي واحد
      }
      if (best) {
        if (!bestSub) console.warn(`[embed:${provider}] استخدمنا الستريم بدون ترجمة عربية (المصدر ما يوفرها)`, best);
        return {
          provider,
          videoUrl: embedProxyUrl('m3u8', best.url, best.headers),
          subUrl: bestSub ? embedProxyUrl('sub', bestSub.url, {}) : null,
          quality: best.quality || '',
          name: best.name || provider,
          noArabicSub: !bestSub,
        };
      }
    } catch (e) {
      console.warn(`[embed:${provider}] فشل الطلب —`, reqUrl, e);
    }
  }
  return null;
}
...
const embedStatus = document.getElementById('ref-embed-status');
const embedResults = document.getElementById('ref-embed-results');
const embedSeasonBox = document.getElementById('ref-embed-season-box');
...
function toggleAccountItem(field) {
  ...
  const { id, mediaType, title, posterPath, year, voteAverage } = currentDetailItem;
...
async function openDetail(id, mediaType) {
  ...
  try {
    ...
    searchEmbedForCurrentItem(); // مصدر إضافي (ترجمة عربية) — أفلام تلقائي، مسلسلات تحتاج موسم/حلقة
    refreshAccountButtonsForCurrentDetail();
...
// ================= بحث embed API (مصدر إضافي — ترجمة عربية) =================
let embedToken = 0;
const embedResultCache = {}; // يمنع إعادة جلب نفس الحلقة/الفيلم أكثر من مرة بنفس الجلسة
const embedSeasonListCache = {}; // قائمة حلقات كل موسم (من TMDB، جلب سريع مرة وحدة لكل موسم)
let embedCurrentSeason = 1;

function resetEmbedBox() {
  embedToken += 1;
  embedStatus.className = 'ref-xtream-status hidden';
  embedStatus.textContent = '';
  embedResults.className = 'ref-xtream-results hidden';
  embedResults.innerHTML = '';
  embedSeasonBox.classList.add('hidden');
  embedSeasonBox.innerHTML = '';
  embedCurrentSeason = 1;
}

function setEmbedStatus(text, kind) {
  embedStatus.classList.remove('hidden');
  embedStatus.className = `ref-xtream-status ${kind || ''}`.trim();
  embedStatus.innerHTML = kind === 'loading'
    ? `<span class="spinner"></span><span>${escapeHtml(text)}</span>`
    : escapeHtml(text);
}

// ================= أفلام: بحث تلقائي فوري، نتيجة وحدة =================
function renderEmbedMovieResult(result) {
  embedResults.classList.remove('hidden');
  const subLabel = result.noArabicSub ? ' (بدون ترجمة عربية)' : ' (ترجمة عربية)';
  embedResults.innerHTML = `
    <div class="ref-xtream-movie-row">
      <span class="ref-xtream-movie-name">${escapeHtml(result.name)}${result.quality ? ' — ' + escapeHtml(result.quality) : ''}${subLabel}</span>
      <div class="ref-xtream-actions">
        <button type="button" class="ref-xtream-action-btn watch" title="مشاهدة مباشرة">${ICON_PLAY}</button>
      </div>
    </div>`;
  embedResults.querySelector('.watch').addEventListener('click', () => goWatch(result.videoUrl, result.subUrl));
}

async function searchEmbedMovie() {
  if (!currentDetailItem) return;
  const { id } = currentDetailItem;
  const cacheKey = `movie:${id}`;
  if (cacheKey in embedResultCache) {
    const cached = embedResultCache[cacheKey];
    if (!cached) setEmbedStatus('ما لقينا نتيجة لهذا الفيلم بهذا المصدر', 'err');
    else { setEmbedStatus(`تم: ${cached.name} (${cached.provider})${cached.noArabicSub ? ' — بدون ترجمة عربية' : ''}`, 'ok'); renderEmbedMovieResult(cached); }
    return;
  }
  const token = ++embedToken;
  setEmbedStatus('جارِ البحث...', 'loading');
  try {
    const result = await fetchEmbedArabicResult('movie', id);
    embedResultCache[cacheKey] = result || null;
    if (token !== embedToken) return;
    if (!result) setEmbedStatus('ما لقينا نتيجة لهذا الفيلم بهذا المصدر', 'err');
    else { setEmbedStatus(`تم: ${result.name} (${result.provider})${result.noArabicSub ? ' — بدون ترجمة عربية' : ''}`, 'ok'); renderEmbedMovieResult(result); }
  } catch (err) {
    if (token !== embedToken) return;
    setEmbedStatus(err.message || 'صار خطأ بالجلب', 'err');
  }
}

// ================= مسلسلات: نجيب قائمة حلقات الموسم كاملة فورًا (من TMDB، سريع)
// وكل حلقة نجيب رابطها الفعلي بس لما تضغطها (هذا اللي ياخذ وقت، تحديدًا هو البحث
// عند مواقع البث). تغيير الموسم من القائمة المنسدلة يجيب حلقات الموسم الجديد تلقائيًا.

async function loadEmbedSeasonEpisodes(seasonNum) {
  const { id } = currentDetailItem;
  const cacheKey = `${id}:${seasonNum}`;
  if (embedSeasonListCache[cacheKey]) return embedSeasonListCache[cacheKey];
  const data = await tmdbFetch(`/tv/${id}/season/${seasonNum}`);
  const episodes = (data && data.episodes) || [];
  embedSeasonListCache[cacheKey] = episodes;
  return episodes;
}

function renderEmbedSeasonDropdown(numberOfSeasons) {
  const seasonNums = Array.from({ length: numberOfSeasons }, (_, i) => i + 1);
  embedSeasonBox.classList.remove('hidden');
  embedSeasonBox.innerHTML = `
    <div class="ref-xtream-season-dropdown" id="ref-embed-season-dropdown">
      <button type="button" class="ref-xtream-season-btn" id="ref-embed-season-btn-el">
        <span id="ref-embed-season-btn-label">الموسم ${embedCurrentSeason}</span>
      ...
      <div class="ref-xtream-season-menu hidden" id="ref-embed-season-menu">
        ${seasonNums.map((s) => `<button type="button" class="ref-xtream-season-option" data-season="${s}">الموسم ${s}</button>`).join('')}
      </div>
    </div>`;
  const dropdown = embedSeasonBox.querySelector('#ref-embed-season-dropdown');
  const btn = embedSeasonBox.querySelector('#ref-embed-season-btn-el');
  const btnLabel = embedSeasonBox.querySelector('#ref-embed-season-btn-label');
  const menu = embedSeasonBox.querySelector('#ref-embed-season-menu');
  btn.addEventListener('click', () => { dropdown.classList.toggle('open'); menu.classList.toggle('hidden'); });
  menu.querySelectorAll('.ref-xtream-season-option').forEach((opt) => {
    opt.addEventListener('click', () => {
      ...
      embedCurrentSeason = s;
      showEmbedSeasonEpisodes(s);
    });
  });
  document.addEventListener('click', (e) => {
    if (!dropdown.contains(e.target)) { dropdown.classList.remove('open'); menu.classList.add('hidden'); }
  });
  menu.querySelector(`[data-season="${embedCurrentSeason}"]`)?.classList.add('active');
}

function embedEpisodeRowHtml(ep) {
  return `
    <div class="ref-xtream-episode-row" data-ep-num="${ep.episode_number}">
      <span class="ref-xtream-episode-name">الحلقة ${ep.episode_number}${ep.name ? ' — ' + escapeHtml(ep.name) : ''}</span>
      <div class="ref-xtream-actions">
        <button type="button" class="ref-xtream-action-btn watch" title="مشاهدة مباشرة">${ICON_PLAY}</button>
      </div>
    </div>`;
...
async function showEmbedSeasonEpisodes(seasonNum) {
  const token = ++embedToken;
  embedResults.classList.remove('hidden');
  setEmbedStatus('جارِ تحميل قائمة الحلقات...', 'loading');
  try {
    const episodes = await loadEmbedSeasonEpisodes(seasonNum);
    if (token !== embedToken) return;
    if (!episodes.length) {
      embedResults.innerHTML = '';
      setEmbedStatus('ما فيه حلقات لهذا الموسم عند TMDB', 'err');
      return;
    }
    embedStatus.classList.add('hidden');
    embedResults.innerHTML = episodes.map(embedEpisodeRowHtml).join('');
    episodes.forEach((ep) => {
      const row = embedResults.querySelector(`[data-ep-num="${ep.episode_number}"]`);
      const btn = row.querySelector('.watch');
      btn.addEventListener('click', () => playEmbedEpisode(seasonNum, ep.episode_number, row, btn));
    });
  } catch (err) {
    if (token !== embedToken) return;
    embedResults.innerHTML = '';
    setEmbedStatus(err.message || 'صار خطأ بجلب الحلقات', 'err');
  }
}

// يجيب رابط الحلقة المحددة فقط (لما تضغطها)، ويشغّلها مباشرة لو لقى ترجمة عربية
async function playEmbedEpisode(seasonNum, episodeNum, row, btn) {
  const { id } = currentDetailItem;
  const cacheKey = `tv:${id}:${seasonNum}:${episodeNum}`;
  if (cacheKey in embedResultCache) {
    const cached = embedResultCache[cacheKey];
    if (cached) { goWatch(cached.videoUrl, cached.subUrl); return; }
    // فشلت قبل كذا — نمسح الكاش ونعيد المحاولة بدل ما تصير معطلة للأبد
    delete embedResultCache[cacheKey];
    row.classList.remove('embed-ep-failed');
  }
  const originalHtml = btn.innerHTML;
  btn.innerHTML = '<span class="spinner"></span>';
  btn.disabled = true;
  try {
    const result = await fetchEmbedArabicResult('tv', id, seasonNum, episodeNum);
    embedResultCache[cacheKey] = result || null;
    if (!result) {
      row.classList.add('embed-ep-failed');
      btn.title = 'ما لقينا ترجمة عربية لهذي الحلقة';
      btn.innerHTML = originalHtml;
      btn.disabled = false;
    } else {
      goWatch(result.videoUrl, result.subUrl);
    }
  } catch (err) {
    btn.innerHTML = originalHtml;
    btn.disabled = false;
    row.classList.add('embed-ep-failed');
    btn.title = err.message || 'صار خطأ';
  }
}

// ================= نقطة البداية: تشتغل تلقائيًا بمجرد فتح تفاصيل أي عمل =================
function searchEmbedForCurrentItem() {
  if (!currentDetailItem) return;
  if (currentDetailItem.mediaType === 'tv') {
    embedCurrentSeason = 1;
    renderEmbedSeasonDropdown(currentDetailItem.numberOfSeasons || 1);
    showEmbedSeasonEpisodes(1);
  } else {
    searchEmbedMovie();
  }
}
...
// يودّي المستخدم لفئة "نتابع" مع تمرير رابط الفيديو تلقائيًا — هذا المكان الوحيد
// اللي ينستخدم فيه الرابط، وما ينعرض/ينحفظ بأي مكان ثاني.
function goWatch(url, subUrl) {
  let target = `../index.html?video=${encodeURIComponent(url)}`;
