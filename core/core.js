...
// ================= استمرارية الجلسة =================
// نخزن محليًا أقل شي ممكن فعليًا (رمز الغرفة والدور بس) — هذا الحد الأدنى مو قابل
// للتفادي: أي جهاز لازم يعرف "أي غرفة" يرجع لها بدون ما يطلب من المستخدم يكتب الرمز
// من جديد كل مرة. كل شي ثاني (الاسم، الوضع، الفيديو الحالي، صحة الغرفة) يجي من
// فايربيس مباشرة كل ما نرجع، مو من تخزين المتصفح — فحتى لو المتصفح مسح كل شيء إلا
// هالمؤشر الصغير، نرجع نبني الحالة كاملة من فايربيس.

function readSession() {
  try { return JSON.parse(localStorage.getItem('wt_session')); } catch (e) { return null; }
}
function writeSession(role, code) {
  localStorage.setItem('wt_session', JSON.stringify({ role, code }));
}
...
confirmCreateBtn.addEventListener('click', () => {
  ...
  roomsDb.collection('rooms').doc(code).set(
    { hostName: name, mode: pendingMode, lastSeenAt: firebase.firestore.FieldValue.serverTimestamp() },
    { merge: true }
  ).catch(() => {});
...
function activateRoomMode(mode) {
  ...
  if (mode === 'games') {
    setTimeout(() => { if (typeof applyGamesBounds === 'function') applyGamesBounds(); }, 30);
  }
}
...
// بـlocalStorage وبترجع تتصل من نفس الرمز. الخروج الفعلي المقصود إله زر "إنهاء الغرفة"
