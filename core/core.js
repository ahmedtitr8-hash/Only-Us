  ...
  dataConn.on('data', (msg) => {
    switch (msg.kind) {
      ...
      case 'mic-state':
        ...
        break;
      // ملاحظة: الشات ومؤشر "يكتب الآن" (مرحلة ١) وتزامن الفيديو بالكامل (مرحلة ٢)
      // انتقلوا لـFirestore (شوف setupFirestoreChatSync أسفل) - ما عادوا يمرون من هنا
      // إطلاقًا. مرحلة ١ كانت تعيد بث آخر فيديو محمّل هنا عند "hello"؛ صار غير محتاج
      // لأن restorePersistedVideoIfAny (قراءة وحدة عند الدخول) + onSnapshot المستمر
      // على currentVideo يغطون نفس الحالة بدون الاعتماد على قناة البيانات.
