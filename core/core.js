...
const nameInputPermanent = document.getElementById('name-input-permanent');
const permanentModeCards = document.querySelectorAll('[data-permanent-mode]');
const permanentReferenceBtn = document.getElementById('permanent-reference-btn');
const entryErrorPermanentMode = document.getElementById('entry-error-permanent-mode');
...
function goToStep(step) {
  ...
  entryStepPermanentSetup.classList.add('hidden');
  entryStepPermanentMode.classList.add('hidden');
...
// ================= الغرفة الدائمة =================
// رمز واحد ثابت يتخزن محليًا (localStorage — عمدًا هنا، بخلاف جلسة الغرفة المؤقتة:
// هذا رمز دائم يبي له الطرفين يتذكرونه دايمًا، مو جلسة مؤقتة). كل مرة تُفتح، تظهر
// قائمة الاختيار من جديد دايمًا — بدون أي دخول تلقائي أو تذكّر لآخر نشاط.

function openPermanentFlow() {
  const savedCode = localStorage.getItem('wt_permanent_code');
  if (savedCode) {
    goToStep(entryStepPermanentMode);
    nameInputPermanent.focus();
  } else {
...
permanentEnterBtn.addEventListener('click', () => {
  ...
  if (code.length < 6) {
    entryErrorPermanentSetup.textContent = 'اكتب الرمز كامل';
...
// حالة محاولة الاتصال بنقطة لقاء دائمة — تُستخدم بس وقت المحاولة نفسها
let isPermanentFlow = false;
let permanentFallbackDone = false;

permanentModeCards.forEach((card) => {
  card.addEventListener('click', () => {
    const name = (nameInputPermanent.value || '').trim();
    if (!name) {
      entryErrorPermanentMode.textContent = 'اكتب اسمك الأول';
      entryErrorPermanentMode.classList.remove('hidden');
      nameInputPermanent.focus();
      return;
    }
    entryErrorPermanentMode.classList.add('hidden');
    ...
    // فعلًا، handlePeerError يتحول تلقائيًا يوصلنا كضيف بدل ما يعيد المحاولة
