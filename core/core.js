...
// يتحقق قبل أي محاولة دخول/إنشاء: لو مرّت 30 دقيقة بدون أي نبضة، الرمز ميت نهائيًا
async function isRoomExpired(code) {
  try {
    const snap = await roomsDb.collection('rooms').doc(code).get();
    if (!snap.exists) return false; // ما فيه سجل = غرفة جديدة، طبيعي تكمل
    ...
    return false; // فشل الفحص (شبكة مثلًا) — ما نمنع الدخول عشان هالمصدر مو ضروري لعمل الغرفة
  }
}
...
if (storedSession) {
  isRoomExpired(storedSession.code).then((expired) => {
    if (expired) {
      clearSession();
...
function activateRoomMode(mode) {
  ...
  // applyPlayerSize معرّفة في watch.js (يتحمّل بعد core.js بشكل غير متزامن، عبر fetch) —
  // ممكن نوصل هذا السطر قبل ما يخلص تحميله (خصوصًا أول دخول على نت بطيء)، فنتحقق إنها
  // موجودة فعلًا قبل ما نناديها عشان ما نكسر تنفيذ الدالة كلها بخطأ صامت.
  if (mode === 'watch') setTimeout(() => { if (typeof applyPlayerSize === 'function') applyPlayerSize(); }, 30);
}
...
function handlePeerError(err) {
  ...
  if (err.type === 'peer-unavailable') {
    ...
    // زي فتح فئة مرجع بالسهم، أو رجعنا من رابط "مشاهدة مباشرة") — السيرفر يفرّغه خلال
    // ثوانٍ قليلة عادة، لكن ممكن يتأخر أكثر على شبكة بطيئة. كانت المحاولة القديمة تستسلم
    // بعد ٣.٥ ثانية بس وتسوي غرفة جديدة تلقائيًا، وهذا بالضبط سبب "دخلتني غرفة ثانية غير
    // اللي كنت فيها" — كانت تتخلى بسرعة وتفصل الطرف الثاني بصمت. نعطيها فرصة أطول بكثير
    // (حوالي ١٨ ثانية) قبل ما نلجأ لهذا الحل الأخير.
    unavailableIdRetries += 1;
    setPeerState('pending', 'جاري إعادة الاتصال بنفس الغرفة');
    if (unavailableIdRetries <= 20) {
      setTimeout(() => beginAsHost(currentRoomCode, myName, roomMode), 900);
      return;
    }
    unavailableIdRetries = 0;
  ...
  dataConn.on('data', (msg) => {
    switch (msg.kind) {
      case 'hello':
        ...
        // أي طرف عنده فيديو محمّل حاليًا يعيد بثه للطرف اللي رجع يتصل (مو المضيف بس - أي
        // وحد منهم ممكن يكون هو اللي حمّل آخر فيديو الحين بعد ما صار التحكم من الطرفين)
        if (typeof currentVideoState !== 'undefined' && currentVideoState) {
