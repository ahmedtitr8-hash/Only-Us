...
function detectVideoType(url) {
  ...
  // نتحقق كمان من ".m3u8" لو ظهرت بأي مكان بالرابط (حتى لو داخل باراميتر مرمّز)،
  // مو بس بنهايته - بعض الروابط تحطه كباراميتر مو كامتداد مباشر
  if (/\.m3u8(?:\?|&|%3f|%26|$)/i.test(url)) return { type: 'hls' };
...
changeVideoBtn.addEventListener('click', () => {
  if (roomMode === 'games') {
    if (typeof exitToPicker === 'function') {
      if (typeof broadcastGameState === 'function') broadcastGameState({ type: 'exit' });
      exitToPicker(true);
    }
  } else {
    toggleVideoUrlPanel();
