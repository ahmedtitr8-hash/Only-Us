...
// ---------- استمرار الفيديو عبر تحديث الصفحة ----------
// نحفظ آخر فيديو محمّل بـsessionStorage (نفس مكان تخزين جلسة الغرفة). سبب هذا: لما
// الصفحة تتحدث (F5) أو نرجع من صفحة مرجع، كل متغيرات جافاسكربت (منها currentVideoState)
// تنمسح، والمشغل يبين فاضي لين الطرف الثاني يرسل الفيديو من جديد عبر رسالة hello — وهذا
// اللي يحس المستخدم معه إنه "دخل غرفة ثانية فاضية" مع إن رمز الغرفة نفسه ما تغيّر أصلًا.
// بإعادة تحميل الفيديو محليًا فورًا من التخزين، المشغل يرجع مباشرة لنفس الشخص اللي حدّث
// صفحته، بدون ما ينتظر الطرف الثاني.
function persistVideoState() {
  try {
    if (currentVideoState) sessionStorage.setItem('wt_video', JSON.stringify(currentVideoState));
    else sessionStorage.removeItem('wt_video');
  } catch (e) { /* تجاهل */ }
}
function readPersistedVideoState() {
  try {
    const raw = sessionStorage.getItem('wt_video');
    return raw ? JSON.parse(raw) : null;
  } catch (e) { return null; }
}
function restorePersistedVideoIfAny() {
  if (currentVideoType) return; // فيديو محمّل فعلًا بهذا التحميل (مثلًا جانا من صفحة مرجع)، لا نستبدله
  const saved = readPersistedVideoState();
  if (saved && saved.url) {
    currentVideoState = saved;
    loadVideo(saved.url, saved.type, false, saved.subUrl);
  }
}
restorePersistedVideoIfAny();
...
changeVideoBtn.addEventListener('click', toggleVideoUrlPanel);
...
function handleIncomingRequestVideo(msg) {
  // إبقاء هذا كتوافق قديم — أي طرف صار يقدر يحمّل الفيديو مباشرة الآن أصلًا
